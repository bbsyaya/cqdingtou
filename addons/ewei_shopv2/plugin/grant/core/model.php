...
class GrantModel extends PluginModel
{
	public function checkplugin($identity)
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$acid = pdo_fetch('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
		...
		if (!(strstr($setting['plugin'], $identity)) && !(strstr($setting['com'], $identity))) {
			$plugin = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_system_plugingrant_log') . ' WHERE uniacid = ' . $acid['acid'] . ' and `identity` = \'' . $identity . '\' ');
