...
class Index_EweiShopV2Page extends PluginMobileLoginPage
{
	public function main()
	{
		...
		if (!(empty($id))) {
			...
			$_SESSION['exchangeGroupId'] = pdo_fetchcolumn('SELECT groupid FROM ' . tablename('ewei_shop_exchange_code') . ' WHERE `key` = :code AND uniacid = :uniacid', array(':code' => $key, ':uniacid' => $_W['uniacid']));
		}
